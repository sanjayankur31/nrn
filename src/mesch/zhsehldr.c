...
ZVEC *
zhhvec (ZVEC *vec, int i0, Real *beta, ZVEC *out, complex *newval)
{
	complex	tmp;
	Real	norm, abs_val;
...
zhhtrvec (
    ...
    int i0,
    ZVEC *in,
    ZVEC *out	/* hh = Householder vector */
)
{
	complex	scale, tmp;
	/* u_int	i; */
...
ZMAT *
zhhtrrows (ZMAT *M, int i0, int j0, ZVEC *hh, double beta)
{
	complex	ip, scale;
	int	i /*, j */;
...
/* zhhtrcols -- transform a matrix by a Householder vector by columns
	starting at row i0 from column j0 -- in-situ */
ZMAT *
zhhtrcols (ZMAT *M, int i0, int j0, ZVEC *hh, double beta)
{
	/* Real	ip, scale; */
	complex	scale;
	int	i /*, k */;
